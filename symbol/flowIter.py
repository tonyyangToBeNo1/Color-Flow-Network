...
class flowIter(mx.io.DataIter):
    ...
    def provide_data(self):
        return [(self.data_name, (self.k, 3, self.crop, self.crop)), ('data_cur', (self.k, 3, self.crop, self.crop))]
        ...
        for i in range(self.image_idx, self.image_idx + self.k):
            if i >= self.size[self.idx]:
                ...
                for j in range(0, self.image_idx + self.k - self.size[self.idx]):
                    img = cv2.imread(self.data[self.idx][self.size[self.idx] - 2])
                    img = img.transpose((2,1,0))
                    img_cur = cv2.imread(self.data[self.idx][self.size[self.idx] - 1])
                    img_cur = img_cur.transpose((2,1,0))
                    images.append(img[:,0:176,0:176])
                    cur.append(img_cur[:,0:176,0:176])
                break
            if not os.path.exists(self.data[self.idx][i]):
                raise 'the image Path is not exists!'
            ...
            if i + 1 >= self.size[self.idx]:
                img_cur = cv2.imread(self.data[self.idx][0]).transpose((2,1,0))
            else:
                img_cur = cv2.imread(self.data[self.idx][i+1]).transpose((2,1,0))
            cur.append(img_cur[:, random_height:random_height+self.crop, random_width:random_width+self.crop])
        
        self.image_idx += self.k
        
        return mx.nd.array(images), mx.nd.array(cur)
    ...
    def next(self):
        if self.iter_next():
            images, cur = self._getdata()
            #print images.asnumpy()[0][0,0,:]
            #labels = self._getlabel()
            img = [images, cur]
            #labels = [labels]
            return mx.io.DataBatch(data=img, provide_data=self.provide_data,  pad=self._getpad)
        else:
            raise StopIteration
    ...
    def infer_shape(self):
        return [(self.batch_size * self.k, 3, self.crop, self.crop), (self.batch_size * self.k, 3, self.crop, self.crop)], []
